...
# My favorite Theroem is the CLT

rm(list = ls()) # clear memory

set.seed(22092008) # set random number generator seed

N<-1000000 # population size (this is truth we will smaple from)

n <- 250 # sample size (size of smaple we will draw for calculation of sample mean)
...
abline(v = 0, col = "red", lwd = 2)
 ...
 # Another (more obvious) are small samples from a bimodel distribution (where the mean bounces around). 
rm(list = ls()) # clear memory
set.seed(123)
