...
cps <- cps2[c(1:3)]
ggpairs(cps)
# you can of course make this prettier. 
...
# You just look at the output. The p-value on the F-Statistic is extremely close to zero. You reject the null that your model has no explanatory power over a simple average. 

# Step 5: Generate the predicted values from Step 3 (Hint: Subtract residuals from ln(wage). Plot the residuals against the predicted values. What do you see?
cps$res <- mrm$resid
...
#  With my little eye, I think I spot some heteroskedastic Residuals! But I am not sure. So I would like to test.


# Step 6: Conduct white test for heteroskedasticity. What do you learn? (use the white_test command)
# This one is a bit tricky. You could do it by hand. 
# I give you some code that works though, using the skedastic package. 
white(mrm, interactions = TRUE)
# The pvalue is pretty small. Less than 5%. I would like to adjust for it. 
# Ok. We could do it by hand. 

res2 <- resid(mrm)^2

# Step 2: auxiliary regression on all regressors, squares, and cross-products
aux <- lm(res2 ~ educ + exper + I(educ^2) + I(exper^2) + I(educ*exper), data = cps)

# Step 3: compute test statistic
n <- nobs(aux)
R2 <- summary(aux)$r.squared
LM <- n * R2
pval <- pchisq(LM, df = length(coef(aux)) - 1, lower.tail = FALSE)

LM
pval
# micdrop. That hashtag is for real folks!!!
